...
// Minimal input type for the tool.
interface AnalyzeRepoInput {
  repoUrl: string;
}

// Minimal output type expected from the tool.
interface AnalyzeRepoOutput {
  repoUrl: string;
  summary: string;
  findings: string[];
}
  ...
  const tools: Tool[] = [
    {
      ...
      description:
        "Analyze a GitHub repository URL and return a stubbed DeepWiki-style summary.",
      inputSchema: {
        type: "object",
        properties: {
          repoUrl: {
...
async function analyzeWithGemini(repoUrl: string): Promise<AnalyzeRepoOutput> {
...
server.setRequestHandler(CallToolRequestSchema, async (request): Promise<any> => {
  ...
  const args = (rawArgs ?? {}) as Partial<AnalyzeRepoInput>;
  ...
  try {
    ...
    output = {
      repoUrl,
      summary:
        "Gemini-backed analysis is temporarily unavailable; this is a static fallback summary.",
      findings: [
